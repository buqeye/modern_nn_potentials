...
class GSUMDiagnostics:
    def __init__(self, nn_interaction, observable, Lambda_b, inputspace, traintestsplit, 
        ...
        self.x = self.inputspace.input_space(**{"deg_input" : self.x_quantity_array, 
                                                "p_input" : E_to_p(self.fixed_quantity_value, interaction = self.nn_interaction),
    ...
    def plot_posteriors_curvewise(self, SGT, DSG, AY, A, D, AXX, AYY, 
        ...
        @ray.remote
        def log_likelihood(gp_fitted, x_interp, p, mesh_points):
            ...
            for order_counter in range(1, order_num + 1):
                ...
                if self.observable_name == "SGT":
                    ...
                    for t_lab_pt, t_lab_mom_pt in zip(t_lab_pts, E_to_p(t_lab_pts, interaction = self.nn_interaction)):
                        ...
                        # sieves the data
                        dsg_data = np.reshape(DSG[:, np.isin(t_lab, t_lab_pt)][..., np.isin(degrees, degrees_pts)], (len(self.nn_orders_full), -1))
                    ...
                    for t_lab_pt, t_lab_mom_pt in zip(t_lab_pts, E_to_p(t_lab_pts, interaction = self.nn_interaction)):
                        ...
                        for so, spin_obs in enumerate(spin_obs_list):
                            # sieves the data
                            spin_data = np.reshape(spin_obs[:, np.isin(t_lab, t_lab_pt)][..., np.isin(degrees, degrees_pts)], (len(self.nn_orders_full), -1))
            ...
            with plt.rc_context({"text.usetex": True}):
                ...
                for i in range(order_num):
                    ...
                    for variable_idx, variable in enumerate(np.roll(variables_array, 1)):
                        ...
                        try:
                            ax_marg_array[variable_idx].axvline(np.roll([variable.user_val for variable in variables_array], 1)[variable_idx], 0, 1, c=gray, lw=1)
                        except:
                            pass
                    ...
                    for joint_idx, joint in enumerate(joint_post_array[:, i]):
                        ...
                        ax_joint_array[joint_idx].text(.99, .99, rf'$\rho$ = {corr_coeff:.2f}', 
                                                    ...
                                                    fontsize = 18)
                        try:
                            ax_joint_array[joint_idx].axvline(np.roll([variable.user_val for variable in variables_array], 1)[comb_array[joint_idx, 0]], 0, 1, c=gray, lw=1)
                        except:
                            pass
                        try:
                            ax_joint_array[joint_idx].axhline(np.roll([variable.user_val for variable in variables_array], 1)[comb_array[joint_idx, 1]], 0, 1, c=gray, lw=1)
                        except:
                            pass
