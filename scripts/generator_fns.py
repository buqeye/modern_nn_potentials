...
def generate_diagnostics(
    ...
    for o, ScaleScheme in enumerate(scale_scheme_bunch_array):
        ...
        for j, x_quantities_tuple in enumerate(x_quantities_tuples):
            ...
            DBunch = ObservableBunch(
                "D", D, E_angle_vals_pair, "D", "dimensionless", nn_interaction="np",
            )
            DBunch_dimensionful = ObservableBunch(
                "D_dimensionful", D, E_angle_vals_pair, "D", "dimensionful", nn_interaction="np",
            )
            AXXBunch = ObservableBunch(
                "AXX", AXX, E_angle_vals_pair, "A_{xx}", "dimensionless", nn_interaction="np",
            )
            AYYBunch = ObservableBunch(
                "AYY", AYY, E_angle_vals_pair, "A_{yy}", "dimensionless", nn_interaction="np",
            )
            ...
            for m, Observable in enumerate(observable_array):
                ...
                for p, PParamMethod in enumerate(p_param_method_array):
                    ...
                    vsquantity_array = np.reshape(np.array([
                        b for b in vsquantity_array if b.name in np.array(input_space_input).flatten()
